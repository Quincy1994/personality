...
class MTHAN():
    ...
    def __init__(self, num_classes, embedding_size, l2_reg_lambda, max_num, max_len):
        ...
        with tf.variable_scope("sent2vec_main", reuse=None):
        ...
        with tf.variable_scope("sent2vec_main", reuse=True):
            # aux
        ...
        with tf.variable_scope("doc2vec_main", reuse=None):
        ...
        with tf.variable_scope("doc2vec_main", reuse=True):
        ...
        #     h_aux = tf.reshape(concat_aux, [-1, 4, self.s_hidden * 2])
        #     # aux = attenten_layer(h_aux, self.s_atten)
        #     aux, alphas = auxAttention(h_aux, self.doc, self.s_atten)
        ...
        with tf.name_scope("loss"):
            ...
            # losses_aux = tf.reduce_mean(losses_aux_1) + tf.reduce_mean(losses_aux_2) + \
            #              tf.reduce_mean(losses_aux_3) + tf.reduce_mean(losses_aux_4)
